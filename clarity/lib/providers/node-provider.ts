...
import { broadcastTransaction, ClarityAbiVariable, ClarityType, ClarityValue, deserializeCV, makeContractCall, noneCV, responseErrorCV, responseOkCV, serializeCV, SignedContractCallOptions, TxBroadcastResultOk, TxBroadcastResultRejected } from '@stacks/transactions';
import fetch from 'cross-fetch';
import { BaseProvider, IProviderRequest } from '.';
import { StacksNetworkConfiguration } from 'clarity/configuration';
import { ClarityAbiMap, ContractInstances, Contracts, cvToValue, getContractIdentifier, parseToCV, SubmitOptions, Submitter, Transaction, TransactionResult, WebTransactionReceipt } from '..';
...
export class NodeProvider implements BaseProvider {
    ...
    makeTx<Ok, Err>(payload: TxPayload): NodeTransaction<any, any> {

        const submit: Submitter<any, any> = async (options: SubmitOptions): Promise<WebTransactionReceipt<any, any>> => {
