...
export interface ArkadikoStxReserveV11Contract {
  addTokensToStack: (tokenAmount: number, metadata: IMetadata) => Transaction<number, number>;
  ...
  collateralizeAndMint: (token: string, tokenString: string, ustxAmount: number, debt: number, sender: string, stackPox: boolean, metadata: IMetadata) => Transaction<number, number>;
  deposit: (token: string, tokenString: string, additionalUstxAmount: number, metadata: IMetadata) => Transaction<boolean, number>;
  ...
  requestStxToStack: (requestedUstx: number, metadata: IMetadata) => Transaction<boolean, number>;
  setTokensToStack: (newTokensToStack: number, metadata: IMetadata) => Transaction<boolean, number>;
  subtractTokensToStack: (tokenAmount: number, metadata: IMetadata) => Transaction<number, number>;
  toggleStacking: (revokedStacking: boolean, ustxCollateral: number, metadata: IMetadata) => Transaction<number, number>;
  ...
  getTokensToStack: (metadata: IMetadata) => Promise<ClarityTypes.Response<number, null>>;
