...
export interface ArkadikoSip10ReserveV11Contract {
    ...
    collateralizeAndMint: (token: string, tokenString: string, ucollateralAmount: number, debt: number, sender: string, stackPox: boolean, metadata: IMetadata) => Transaction<number, number>;
    deposit: (token: string, tokenString: string, additionalUcollateralAmount: number, metadata: IMetadata) => Transaction<boolean, number>;
