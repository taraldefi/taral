...
export function hotpVerifyDelta(options: HotpOptions & { token: string }): number | null {
...
// secret generation
export function generateSecret(options: GenerateSecretOptions = {}):{ ascii: string; hex: string; base32: string; otpauth_url?: string } {
  ...
  // otpauth
  export function otpauthURL(options: { type: string; secret: string; label: string; issuer?: string; counter?: number; digits?: number; period?: number; }): string {
    const type = options.type || 'totp';
