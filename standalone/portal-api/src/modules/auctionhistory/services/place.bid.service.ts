...
export class PlaceBidService extends BaseService {
  ...
  public async placeBid(placeBidModel: PlaceBid): Promise<void> {
    ...
    if (
      auction.bids.find(
        (b) => b.bidder === bid.bidder && b.amount == bid.amount,
      )
    ) {
      // same bid, do nothing
    ...
    await this.insertIntoHistory(
      ...
      bid,
      'insert',
      (entity: AuctionBidEntity, history: AuctionBidHistoryEntity) => {
      ...
      (entity: AuctionBidHistoryEntity) =>
        this.auctionBidHistoryRepository.save(entity),
    );
