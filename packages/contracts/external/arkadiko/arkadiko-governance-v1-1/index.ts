...
export const nodeArkadikoGovernanceV11Contract = (
  provider: BaseNodeProvider
) => {
  const contract = nodeProxy<ArkadikoGovernanceV11Contract>(
...
export const webArkadikoGovernanceV11Contract = (provider: BaseWebProvider) => {
  const contract = webProxy<ArkadikoGovernanceV11Contract>(
    ArkadikoGovernanceV11Interface,
    provider
  );
  return contract;
