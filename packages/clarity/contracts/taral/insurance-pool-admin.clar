...
;; stx address: ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM
;; btc address: mqVnk6NPRdhntvfm4hh9vvjiRkFDUuSYsH
(define-constant pool-pox-address {hashbytes: 0x6d78de7b0625dfbfc16c3a8a5735f6dc3dc3f2ce, version: 0x00})
...
(define-public (submit-reward-tx (block { version: (buff 4), parent: (buff 32), merkle-root: (buff 32), timestamp: (buff 4), nbits: (buff 4), nonce: (buff 4), height: uint })
  ...
  (match (contract-call? .insurance-pool-audit submit-reward-tx block tx proof)
    success (begin
...
(define-read-only (get-rewards (user principal) (cycle uint))
  (let ((total (unwrap! (map-get? total-stacked-stxs cycle) err-invalid-cycle))
          ...
          (rewards (+ (contract-call? .insurance-pool-audit get-rewards cycle) (get-unaudited-rewards cycle))))
...
(define-read-only (get-reward-balance (cycle uint))
  (- (to-int (contract-call? .insurance-pool-audit get-rewards cycle))
