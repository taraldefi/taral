...
describe("Should test purchase order nft flows", () => {
  it("Should ensure one is able to MINT and get last token ID", () => {
    const mintResult = simnet.callPublicFn(
      ...
      "mint",
      [Cl.uint(1), Cl.standardPrincipal(WALLET_1)],
      DEPLOYER,
    );
    ...
    it("Should ensure one is able to transfer NFT from one account to another", () => {
      ...
      expect(nftTransferEvent.asset_identifier).toStrictEqual(
        `${DEPLOYER}.taral-purchase-order-nft::purchase-order-nft`,
      );
      ...
      expect(getNewOwnerResult.result).toBeOk(
    ...
    it("Should ensure one is able to burn NFT by providing the token ID", () => {
      ...
      expect(nftBurnEvent.asset_identifier).toStrictEqual(
        `${DEPLOYER}.taral-purchase-order-nft::purchase-order-nft`,
      );
